...
guiItemDetails.start = function(url,selectedItem,topLeftItem) {	
	...
	if (this.ItemData.LocationType != "Virtual") {
		if (this.ItemData.UserData.PlaybackPositionTicks > 0) {
			...
			this.menuItems1.push("Resume - "+support.convertTicksToTimeSingle(resumeTicksSamsung));
		}
		this.menuItems1.push("Play");
	...
	if (this.ItemData.Type == "Episode") {
		...
		"<div class='guiItemDetails-ItemContainer'>" + 
			...
			"<div id='guiItemDetails-MenuContainer2' class='guiItemDetails-MenuContainer'></div>" +
		"</div>" +
		...
		if (this.EpisodeData.Items.length > 1) {
			this.menuItems2.push("More Episodes");
		}
		...
		this.menuItems1.push("Watched");
		this.menuItems1.push("Favourite");
		...
		if (this.ItemData.ImageTags.Primary) {			
			var imgsrc = server.getImageURL(this.ItemData.Id,"Primary",600,338,0,false,0);
		...
		//Set Episode Primary Image
		if (this.ItemData.ImageTags.Primary) {			
			var imgsrc = server.getImageURL(this.ItemData.Id,"Primary",338,600,0,false,0);
			document.getElementById("ShowImage").style.backgroundImage="url('" + imgsrc + "')";
		}
	}
	...
	for (var index = 0; index < this.menuItems1.length; index++) {
		document.getElementById("guiItemDetails-MenuContainer1").innerHTML += "<span id='guiItemDetails-MenuItem" + index + "' class='guiItemDetails-MenuItem'>"+this.menuItems1[index]+"</span>";			
	}
	...
	this.selectedMenuItem1 = 0;
	this.updateSelectedMenuItems1();
...
//Menu
guiItemDetails.updateSelectedMenuItems1 = function() {
	support.updateSelectedMenuItems(this.menuItems1.length,this.selectedMenuItem1,"guiItemDetails-MenuItem highlightBackground","guiItemDetails-MenuItem","guiItemDetails-MenuItem");
}
...
guiItemDetails.processLeftKey = function() {
	...
	case (remotecontrol.MENU):
		if (this.selectedMenuItem1 > 0) {
			this.selectedMenuItem1--;
			this.updateSelectedMenuItems1();
		}
		break;	
...
guiItemDetails.processRightKey = function() {
	switch (this.selectedRow) {
	case (remotecontrol.MENU):
		if (this.selectedMenuItem1 < this.menuItems1.length-1) {
			this.selectedMenuItem1++;
			this.updateSelectedMenuItems1();
		}
		break;
